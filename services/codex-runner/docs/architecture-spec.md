...
This persists durable task state. The current implementation is SQLite-backed through the shared `@bensigo/plato-db` foundation, but the contract stays intentionally narrow so the storage backend can evolve without changing service behavior.
